...
import { Button, Badge, Drawer, Checkbox, Stack } from "rsuite";
    ...
    return (
      <div className="p-4">
        ...
        <Drawer open={open} onClose={() => this.setState({ open: false })} size="xs">
          <Drawer.Header>
            <Drawer.Title>Filtro</Drawer.Title>
          </Drawer.Header>
          <Drawer.Body style={{ padding: "30px" }}>
            <Row gutterWidth={0}>
            <Col md={12}>
                <div className="form-control">
                  <AutoComplete label="Empresa" value={this.state?.company} text={(item) => `${item.surname}`} onChange={(company) => this.setState({ company })} onSearch={async (search) => await Search.company(search)} autoFocus>
                    <AutoComplete.Result>
